...
public class TalkRepositoryImpl extends BasicRepositoryImpl<Talk> implements TalkRepository {
    ...
    public void save(Talk talk) {
        save("sourceUserId", talk.getSourceUserId(), "targetUserId", talk.getTargetUserId(), "text", talk.getText(), "creationTime", null);
    }

    protected Talk toCreate(ResultSetMetaData metaData, ResultSet resultSet) throws SQLException {
        if (!resultSet.next()) {
            return null;
        }
        ...
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM Talk WHERE sourceUserId=? OR targetUserId=? ORDER BY creationTime DESC")) {
                ...
                try (ResultSet resultSet = statement.executeQuery()) {
                    Talk talk;
                    while ((talk = toCreate(statement.getMetaData(), resultSet)) != null) {
                        talks.add(talk);
                    }
                }
            }
